...
$customuser->id = $newuserid;  // link to core user
$customuser->batch  = $user->profile['batch'] ?? '';
$customuser->phone1 = $user->profile['phone1'] ?? '';
$customuser->degree = $user->profile['degree'] ?? '';
$customuser->department = $user->profile['department'] ?? '';
$customuser->cgpa = $user->profile['cgpa'] ?? '';
$customuser->yearofpassedout = $user->profile['yearofpassedout'] ?? '';
$customuser->questiontype = $user->profile['questiontype'] ?? '';
$customuser->relocate = $user->profile['relocate'] ?? '';
$customuser->backlog = $user->profile['backlog'] ?? '';
$customuser->immediatejoin = $user->profile['immediatejoin'] ?? '';
$customuser->offerinhand = $user->profile['offerinhand'] ?? '';
$customuser->city = $user->city ?? '';
$customuser->country = $user->country ?? 'IN';

// Insert only custom fields, NOT username or password
$DB->update_record('user', $customuser);
