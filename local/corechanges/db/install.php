...
/**
 * Install hook for local_corechanges.
 * Runs when the plugin is installed for the first time.
 */
function xmldb_local_corechanges_install() {
    global $DB;
    ...
    $field = new xmldb_field('fathername', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'offerinhand');
    if (!$dbman->field_exists($table, $field)) {
        $dbman->add_field($table, $field);
    }
    return true;
